...
char tag2[tagLength] = "000C9523A11B"; // rat 2 tag
...
void loop(){
  ...
  // Auto updates the display every 100 loops. Important for returning to normal function after reading an unknown tag. 
  tick++;
  if (tick % 150 == 0){
    display.clearDisplay();
    updateDisplayRat1();
...
void checkTag(char tag[]){
  ...
  if (compareTag(tag, tag1)) { // if senses tag 1
    start = millis();
    while ((compareTag(tag, tag1)) || (((millis()-start) < 800))){ // While the tag present is being sensed
